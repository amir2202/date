...
class ProfileInfoWrapper extends StatefulWidget {
  ...
  ProfileInfoWrapper({Key key, @required this.disownCallback, @required this.notifier, @required this.navigatorKey});
...
class ProfileInfoWrapperState extends State<ProfileInfoWrapper> {
  ...
  Map<String, WidgetBuilder> _routeBuilders(BuildContext context, String name, String imageUrl, List<String> pictureUrls, int totalViews, int totalLikes) {
    return {
      ...
      ProfileInfoWrapperState.profile: (context) => ProfilePage(tabCallback: (a, b) {}, disownCallback: widget.disownCallback, notifier: widget.notifier, myProfile: false, name: name, imageUrl: imageUrl, pictureUrls: pictureUrls, totalViews: totalViews, totalLikes: totalLikes),
    };
  }
